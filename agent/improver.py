...
async def _upload_job(
    client: httpx.AsyncClient, job: Job, master_resume_id: str | None = None
) -> str | None:
    """Upload JD text. Returns RM job_id string or None."""
    payload: dict = {"job_descriptions": [job.description]}
    if master_resume_id:
        payload["resume_id"] = master_resume_id
    try:
        resp = await client.post("/api/v1/jobs/upload", json=payload)
        resp.raise_for_status()
