...
async def _upload_job(
    client: httpx.AsyncClient, job: Job, master_resume_id: str
) -> str | None:
    """Upload JD text. Returns RM job_id string or None.

    Passes resume_id so RM can associate the uploaded JD with the master resume
    (JobUploadRequest.resume_id field — optional but used by RM for context).
    """
    try:
        resp = await client.post(
            "/api/v1/jobs/upload",
            json={"job_descriptions": [job.description], "resume_id": master_resume_id},
        )
        resp.raise_for_status()
